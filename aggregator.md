...
С 15 апреля 2026 года действует новая политика по написанию и подключению новых платёжных шлюзов:

1.  **Разработку ведёт наша команда:** Для ускорения процесса, повышения качества и соответствия стандартам платформы, все новые шлюзы разрабатываются и интегрируются нашей внутренней командой разработчиков. Мы больше не принимаем код от внешних команд для интеграции.
2.  **Условия подключения:**
    *   **1 шлюз в месяц** — бесплатно.
    *   **Последующие шлюзы** в том же месяце подключаются на платной основе согласно тарифной сетке (запросите у вашего менеджера).
3.  **Процесс утверждения:**
    *   Все запросы на интеграцию новых шлюзов должны быть согласованы и подтверждены через руководителя технического отдела (@U092UL7LTR7 в Slack).
    *   Это правило не распространяется на White-labels "Протокол", "Агрипей" и "Ковенант", для которых интеграции производятся в приоритетном порядке по старым правилам.

Это изменение введено для оптимизации ресурсов, уменьшения времени на тестирование и отладку, и для обеспечения стабильности платформы.
