...
### Реквизиты трейдера — отображение полей (INB-660)

В модальных окнах **создания** и **редактирования реквизита** поле «Номер телефона / карты / счёта» теперь отображается корректно для всех методов оплаты.

**Что изменилось:**
- Поле номера счёта для корейских методов оплаты приведено к единому формату отображения — аналогично СБП и другим методам.
- Устранена визуальная ошибка: поле более не обрезается и не теряет стили в модальных окнах ЛК трейдера.

> **Jira:** INB-660 | **Версия:** hotfix/v1.24.26
...
## Асинхронная проверка платежей через агрегатор

`DealAggregatorCheckJob` — yii2-queue job, которая проверяет существование платежа в AGGREpay по `order_id` и при необходимости отменяет ордер.

### Когда запускается

Запускается асинхронно через очередь при возникновении расхождений между статусами в Trade и агрегаторе.

### Алгоритм

1. Запрашивает платёж из AGGREpay по `order_id` через `AggrepayService::getTransactions()`
2. **До 5 попыток** с интервалом 2 секунды (`maxRetries=5`, `retryIntervalSec=2`)
3. При каждой попытке возможны три исхода:
   - **`found`** — платёж найден в агрегаторе:
     - Если `state=failed` → отменяет ордер (status=6)
     - Если `transaction_id` не совпадает ни с одним инвойсом → отменяет ордер
     - Иначе → ордер корректен, завершает проверку
   - **`not_found`** — платёж не найден, продолжает попытки
   - **`error`** — HTTP ошибка при запросе к агрегатору, продолжает попытки
4. Если все попытки исчерпаны → **отменяет ордер** (status=6) и отправляет callback агрегатору

### Отмена ордера (`cancelDeal`)

- Устанавливает `deal.status = 6` (отменён)
- Отправляет callback агрегатору через `AggrepayService::sendColback()`
- Весь процесс трассируется через OpenTelemetry

### Конфигурация

```
AGREPAY_BASE_URL=https://...
AGREPAY_AUTH_TOKEN=...
```

### Трассировка (Grafana)

В Grafana span называется `DealAggregatorCheckJob`. Дочерние spans: `check_aggregator_attempt_N`, `cancel_deal`.
